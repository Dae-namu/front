...
const CardSection = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        //const response = await fetch(`${process.env.REACT_APP_API_URL}/dramas`);
        const response = await fetch('/dramas'); // 로컬 개발 환경에서는 이 경로를 사용
        ...
        <div key={sectionIndex} className="relative">
          ...
          >
            {section.items.map((item, idx) => (
              ...
              >
                <img
                  src={item.thumbnailUrl || 'https://via.placeholder.com/240x320?text=No+Image'}
                  alt={item.title}
